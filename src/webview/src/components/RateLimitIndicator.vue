...
interface Props {
  utilization?: number  // 0–1; undefined when API doesn't return it
...
const percentage = computed(() =>
  props.utilization != null ? Math.max(0, Math.min(1, props.utilization)) * 100 : undefined
)

const strokeOffset = computed(() => {
  if (percentage.value == null) return circumference.value  // empty circle
  return circumference.value - (percentage.value / 100) * circumference.value
})

const strokeColor = computed(() => {
  if (props.status === 'rejected' || (percentage.value != null && percentage.value >= 95)) {
    return 'color-mix(in srgb, var(--vscode-chart-red) 92%, transparent)'
  }
  if (props.status === 'allowed_warning' || (percentage.value != null && percentage.value >= 75)) {
...
const tooltipText = computed(() => {
  ...
  if (percentage.value != null) {
    const formatted = `${percentage.value % 1 === 0 ? Math.round(percentage.value) : percentage.value.toFixed(1)}%`
    text = `${formatted} of ${window} limit used`
  } else {
    text = `${window} rate limit active`
  }
  if (props.resetsAt) {
